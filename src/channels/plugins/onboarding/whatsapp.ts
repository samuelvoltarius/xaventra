...
function setWhatsAppDmPolicy(cfg: typeof novaConfig, dmPolicy: DmPolicy): typeof novaConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: typeof novaConfig, allowFrom?: string[]): typeof novaConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: typeof novaConfig, selfChatMode: boolean): typeof novaConfig {
...
async function detectWhatsAppLinked(cfg: typeof novaConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so nova can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for nova" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("nova channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
