...
function setSlackDmPolicy(cfg: typeof novaConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "nova";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for nova`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/clawd",
          description: "Send a message to nova",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: typeof novaConfig, allowFrom: string[]): typeof novaConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
