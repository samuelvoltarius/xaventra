...
const resolveStateDir = vi.fn().mockReturnValue("/tmp/nova-state");
...
describe("github-copilot token", () => {
  beforeEach(() => {
    ...
    resolveStateDir.mockReturnValue("/tmp/nova-state");
  });
  ...
  it("uses cache when token is still valid", async () => {
    ...
    loadJsonFile.mockReturnValue({
      token: ["cached;proxy-ep=", "proxy.example.com;"].join(""),
  ...
  it("fetches and stores token when cache is missing", async () => {
    ...
    const fetchImpl = vi.fn().mockResolvedValue({
      ...
      status: 200,
      json: async () => ({
        token: ["fresh;proxy-ep=", "https://proxy.contoso.test;"].join(""),
