...
const originalClientId = process.env.QWEN_OAUTH_CLIENT_ID;

beforeEach(() => {
  process.env.QWEN_OAUTH_CLIENT_ID = "test-public-client";
});

afterEach(() => {
  vi.unstubAllGlobals();
  globalThis.fetch = originalFetch;
  if (originalClientId === undefined) delete process.env.QWEN_OAUTH_CLIENT_ID;
  else process.env.QWEN_OAUTH_CLIENT_ID = originalClientId;
});
...
describe("refreshQwenPortalCredentials", () => {
  ...
  it("fails closed when this node has no OAuth client configuration", async () => {
    delete process.env.QWEN_OAUTH_CLIENT_ID;
    await expect(
      refreshQwenPortalCredentials({
        access: "old-access",
        refresh: "old-refresh",
    ...
    ).rejects.toThrow("QWEN_OAUTH_CLIENT_ID is not configured");
  });
});
