...
describe('action intent evidence gate', () => {
    ...
    it('routes natural project and workspace questions through file tools', () => {
        ...
        expect(detectActionIntent('Lies /tmp/a.txt und /tmp/b.txt mit read_file')).toEqual({ requiresTool: true, kind: 'file' })
        expect(detectActionIntent('Read C:\\work\\a.txt with read_file')).toEqual({ requiresTool: true, kind: 'file' })
