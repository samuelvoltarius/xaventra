...
export async function upstreamUpdateCommand(args: string, permission: string, source = configuredUpstreamSource()): Promise<string> {
    ...
    if (action === 'status') return formatUpstreamStatus(await source.status())
    ...
    return `${report}\n\n❌ Aktivierung gesperrt: Der GitHub-Paketpfad ist noch nicht mit einem unabhängig autorisierten Aktivierungscontroller verbunden. SSH-Mesh-Profile oder ein Host-Agent für Docker-Inventar ersetzen diesen nicht. Kein Rollout gestartet.`
}
