...
try {
    ...
    for (const test of DOCTOR_QUALITY_CASES) {
        const started = Date.now()
        const result = { id: test.id, schemaValid: false, runtimeAccepted: false, semanticPassed: false, findings: [] }
        try {
            const raw = await engine.complete(buildDiagnosePrompt({ error: test.error }), {
                systemPrompt: contract.DOCTOR_DIAGNOSIS_INSTRUCTIONS, jsonSchema: contract.DOCTOR_FIX_PLAN_GRAMMAR,
            ...
            try { contract.parseDoctorDiagnosis(raw, { reportedError: test.error }); result.runtimeAccepted = true }
